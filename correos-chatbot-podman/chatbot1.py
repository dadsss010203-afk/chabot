...
# ─────────────────────────────────────────────
#  NOMINATIM — fallback de coordenadas
# ─────────────────────────────────────────────
_coords_cache: dict = {}
...
def _cargar_secciones(chunks: list, chunk_ids: list) -> int:
    """Carga secciones_home.json en los chunks. Retorna cantidad de secciones agregadas."""
    total = 0
    try:
        if os.path.exists("data/secciones_home.json"):
            ...
            for seccion_nombre, items in secciones.items():
                if items:
                    ...
                    total += 1
            print(f"📋 {total} secciones indexadas (Servicios, Aplicativos, etc.)")
...
def detectar_consulta_ubicacion(texto: str) -> dict | None:
    ...
    # Pide ubicación pero sin ciudad específica
    return {"ciudad": None}
...
if collection.count() == 0:
    ...
    # ── BUG FIX: secciones también en la indexación inicial ───────
    _cargar_secciones(chunks, chunk_ids)
...
def limpiar_respuesta(texto: str) -> str:
    """Elimina bloques <think>...</think> que qwen3 puede incluir en el output."""
...
def chat():
    ...
    # ── 3. Detectar intención (solo regex, sin Ollama) ────────────
    if geo is None:
