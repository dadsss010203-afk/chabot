...
OLLAMA_TIMEOUT      = int(os.environ.get("OLLAMA_TIMEOUT",      "120"))   # FIX: era 600
...
# ─────────────────────────────────────────────
# FIX: saludos no existían → "holis" iba a RAG y devolvía sin_info
...
# FIX: scheduler como global para poder hacer shutdown correctamente
scheduler = None
...
def chat():
    ...
    # ── 1. Saludo → respuesta inmediata sin Ollama  (FIX)
    if es_saludo(pregunta):
        return jsonify({"response": t["saludo"]})

    # ── 2. Despedida
    if es_despedida(pregunta):
        historiales.pop(sid, None)
        return jsonify({"response": t["despedida"], "despedida": True})
    ...
    # ── 4. Detectar intención de ubicación (solo regex)
    if geo is None:
    ...
    if geo is not None:
        if geo.get("ciudad") is None and "nombre" not in geo:
            ...
            return jsonify({"response": t["pedir_ciudad"].format(ciudades=nombres, cidades=nombres)})
        ...
        resp_json = {"response": texto_resp}
        if lat and lng:
            resp_json["ubicacion"] = {
                "nombre": geo.get("nombre", ""), "direccion": geo.get("direccion", ""),
                "telefono": geo.get("telefono", ""), "email": geo.get("email", ""),
                "horario": geo.get("horario", ""), "lat": lat, "lng": lng, "maps_url": maps_url,
            }
    ...
    try:
        ...
        contexto = "\n\n".join(results["documents"][0])[:800]
    except Exception as e:
        return jsonify({"error": f"Error en búsqueda: {e}"}), 500

    hora = get_hora_bolivia()

    # FIX: system prompt como rol "system", no como "user"
    sistema = (
...
def listar_sucursales():
    ...
    for s in SUCURSALES:
        ...
        lng = s.get("lng")
        resultado.append({
            "nombre": s.get("nombre", ""), "direccion": s.get("direccion", ""),
            "telefono": s.get("telefono", ""), "email": s.get("email", ""),
            "horario": s.get("horario", ""), "lat": lat, "lng": lng,
            "maps_url": generar_maps_url(lat, lng) if lat and lng else None,
        })
...
def status():
    try:
        ...
        ollama_ok = False
    return jsonify({
        "status": "ok", "chunks": collection.count(), "modelo": LLM_MODEL,
        "ollama": ollama_ok, "sesiones_activas": len(historiales),
...
def initialize_chatbot():
    ...
    # FIX: scheduler guardado como global para shutdown correcto
...
if __name__ == "__main__":
    ...
    try:
        ...
        # FIX: shutdown correcto usando variable global
